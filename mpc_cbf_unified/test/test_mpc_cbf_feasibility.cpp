...
// empty. Implement alongside the solver (IMPLEMENTATION_GUIDE.md §8.1).
//
...
protected:
  void SetUp() override
  {
    // TODO(deepseek): fill mpc_config_ and cbf_config_ with the defaults from
...
{
  // TODO(deepseek): gamma = 0.0, -0.1 and 1.5 must all make initialize()
...
{
  // TODO(deepseek): kRelaxedDecay with omega_max * gamma > 1 must be rejected —
...
{
  // TODO(deepseek): Q of length nx-1 -> initialize() false.
  GTEST_SKIP() << "not implemented";
}
...
{
  // TODO(deepseek): barrierValue() > 0 strictly outside the inflated obstacle,
...
{
  // TODO(deepseek): for 100 random states, |barrierValue(...) - h from
...
{
  // TODO(deepseek): status == kSuccess; x_pred has N+1 columns; u0 within
...
{
  // TODO(deepseek): for the returned prediction, verify
...
{
  // TODO(deepseek): 200-step closed-loop rollout applying u0 to the true
...
{
  // TODO(deepseek): roll out with gamma = 0.1 and gamma = 0.9 from the same
...
{
  // TODO(deepseek): same scenario with a short horizon (N = 3). kDistanceOnly
...
{
  // TODO(deepseek): pick a start state and gamma for which kFixedDecay returns
...
{
  // TODO(deepseek): drive the vehicle straight at the obstacle; assert
...
{
  // TODO(deepseek): construct an infeasible instance (start inside the
...
{
  // TODO(deepseek): NaN in x0 -> kNanDetected, no crash, no acados call.
  GTEST_SKIP() << "not implemented";
}
...
{
  // TODO(deepseek): pass kMaxObstacles + 4 obstacles; the nearest kMaxObstacles
...
{
  // TODO(deepseek): solve twice from the same state; the second call (warm
...
{
  // TODO(deepseek): 100 solves with use_rti = true; assert the 95th percentile
...
{
  // TODO(deepseek): install a global operator new counter, warm up with one
