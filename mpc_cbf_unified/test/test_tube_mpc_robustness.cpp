...
//
// SKELETON — see IMPLEMENTATION_GUIDE.md §8.2.
//
...
{
  // TODO(deepseek): for the unit box, support(d) == ||d||_1 for 20 random d.
  GTEST_SKIP() << "not implemented";
}

TEST(PolytopeTest, MinkowskiSumOfBoxesIsABox)
{
  // TODO(deepseek): box(a) (+) box(b) has the same vertices as box(a + b).
  GTEST_SKIP() << "not implemented";
}

TEST(PolytopeTest, PontryaginDifferenceIsInverseOfSumForBoxes)
{
  // TODO(deepseek): (box(a) (+) box(b)) (-) box(b) == box(a) within 1e-9.
  GTEST_SKIP() << "not implemented";
}

TEST(PolytopeTest, PontryaginDifferenceCanBeEmpty)
{
  // TODO(deepseek): box(0.1) (-) box(0.5) must report isEmpty(). The tube
...
{
  // TODO(deepseek): duplicate every half-space, remove redundancies, sample
...
{
  // TODO(deepseek): closed-form support >= max over 10000 sampled points, and
  // within 1e-6 of it.
  GTEST_SKIP() << "not implemented";
}
...
{
  // TODO(deepseek): reduceOrder(n + 2) must contain every sample of the
...
{
  // TODO(deepseek): A_cl = diag(0.5, 0.5), W = box(0.1). Assert converged,
...
{
  // TODO(deepseek): spectral radius 1.1 -> converged == false, no hang.
  GTEST_SKIP() << "not implemented";
}

TEST(RpiTest, ResultIsRobustPositivelyInvariant)
{
  // TODO(deepseek): isRobustPositivelyInvariant(A_cl, Omega, W) == true, and a
...
{
  // TODO(deepseek): discreteLqrGain on the double integrator gives A + B K with
...
protected:
  void SetUp() override
  {
    // TODO(deepseek): build the configs from config/tube_mpc_params.yaml
...
{
  // TODO(deepseek): W large enough that U (-) K Omega is empty must make
...
{
  // TODO(deepseek): tighteningFor() >= 0 everywhere; halving W's generators
...
{
  // TODO(deepseek): for the quadratic barrier, the support-function tightening
...
{
  // TODO(deepseek): 200-step closed loop with w_k chosen adversarially at each
...
{
  // TODO(deepseek): 50 seeds x 200 steps of uniform samples from W; assert
...
{
  // TODO(deepseek): the ablation. tighten_mode = kNone under the same
...
{
  // TODO(deepseek): log e_k = x_k - z_k across the rollout; assert
...
{
  // TODO(deepseek): u_applied within [u_min, u_max] at every step *without*
