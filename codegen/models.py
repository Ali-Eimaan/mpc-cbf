...
Implement per .deepseek/04_MODELS.md.
"""
...
# TODO(deepseek §4): import casadi as ca, numpy as np, and
...
class ModelSpec:
    ...
    # TODO(deepseek §4): continuous-time f(x, u) as a CasADi function; the
...
def double_integrator_2d() -> ModelSpec:
    ...
    TODO(deepseek §4): the only model with an exact discretisation — return the
...
def discretise(spec: ModelSpec, dt: float, method: str = "rk4"):
    ...
    TODO(deepseek §4): explicit RK4 by default, exact ZOH when the model is
...
def linearised_discrete(spec: ModelSpec, dt: float, x_op=None, u_op=None):
    ...
    TODO(deepseek §4): used by the tube solver for K and the RPI set. For the
...
def barrier_expression(spec: ModelSpec, x, obstacle_params):
    ...
    TODO(deepseek §4): this is THE definition of h in the repo. The C++
    ...
    [px, py, pz, vx, vy, vz, radius] — see .deepseek/05_CODEGEN.md §5.3 for the layout.
    """
    raise NotImplementedError
